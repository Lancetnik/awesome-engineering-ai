...
- [Awesome Engineering AI ](#awesome-engineering-ai-)
  ...
  - [Structural code search: grep replacements](#structural-code-search-grep-replacements)
...
- **[writing-for-agents](https://github.com/mattpocock/skills/blob/main/skills/productivity/writing-for-agents/SKILL.md)** — a skill for writing anything an agent reads: a skill, an `AGENTS.md` / `CLAUDE.md`, a doc it only reaches through a pointer. The conventions that separate a skill the agent actually invokes from one it ignores, plus the wording of the pointer that decides whether the material gets reached at all. Start here before hand-rolling your first. (Renamed from `writing-great-skills`.)
...
- ❌ **[superpowers](https://github.com/obra/superpowers)** — brainstorm with probing questions → plan mode → TDD execution, through hard checkpoints. Ran it for months and eventually removed it, and the reason is not a missing feature: **it inserts itself and builds a flow on its own**, so you end up negotiating with the harness instead of working. The contrast that made the decision is [mattpocock/skills](https://github.com/mattpocock/skills) above — there you always say explicitly what you are doing right now, and nothing starts a phase for you. Two costs on top: by default it commits every plan and research note it generates into your repo, and a plan is an intermediate artifact that belongs in the bin after the merge rather than in git history; and it is the most token-hungry option in this section, with independent reports of frontier models looping design → plan → re-plan instead of writing code, and of roughly half the tokens and a third of the wall-clock coming back once it is switched off. Kept on the list, and the disagreement is real: people run it on commercial projects for half a year and call the load acceptable. If you want the phases enforced for you, this is still the one — that is exactly what I stopped wanting.
...
## Structural code search: grep replacements

The agent's default move on an unfamiliar repo is to grep, read whole files, and grep again — which is where a large share of a session's tokens and wall-clock actually go. Everything here replaces that scan with a structure it can query: an AST, a symbol index, a call graph, a language server. Treat the section as a live question rather than a shelf of answers — nothing here is a settled win for me, the failure modes below are from real runs, and a proper A/B (one big repo, one fixed question, same task across tools) is the only thing that will separate them.
...
- 🧪 **[codegraph](https://github.com/colbymchenry/codegraph)** — builds an AST graph of the project via tree-sitter: symbols, calls, dependencies. Instead of blind `grep`, the agent asks precise questions — who calls this, what breaks if I change it, show me the signature — and gets a structural answer in milliseconds. ⚠️ Works, but the bill is real: on a mid-sized Python framework it spent about two minutes and ~100k tokens mapping the base before every task, and the graph front-loaded the reading rather than replacing it — the agent queried codegraph and then went to read the files its answer pointed at anyway. The case where it clearly earns the index: tracing a call chain across classes, where plain grep fans out over dozens of files and one graph query returns the chain.
- 🧪 **[Graphify](https://github.com/Graphify-Labs/graphify)** — same idea, wider input: code, SQL schemas, infra, docs, articles, images and video into one queryable knowledge graph. Works as a skill across Claude Code, Codex, OpenCode, Cursor and Gemini CLI. ⚠️ Same picture from the same run, plus a cost of its own: the usage instructions alone run to roughly 20k tokens of context, which is a strange bill for a tool sold on saving context. Useful, not yet efficient.
...
Once the agent has found the right place (the section above), two bills remain: how much of what came back reaches the model, and whether any of it survives to the next session. Compression and memory live here.
...
- **[LeanCTX](https://github.com/yvgude/lean-ctx)** — the section in one binary, which is either the point of it or the risk. It decides what the agent reads (a cached re-read of a file costs about 13 tokens instead of 2000, a compressed `git status` around 120 instead of 800), compresses each request through an optional local proxy that keeps the prompt cache intact, remembers what was learned between chats and guards what the agent touches — with a ledger of saved tokens rather than a claim. Everything else here picks one of those four jobs; take this one if you would rather run a single local Rust binary than assemble a stack, and the others if you want to keep the pieces separable.
- **[benjamin-plus](https://github.com/JetBrains/benjamin-plus-skill)** — the odd one out here: no index, no graph, no proxy, just ~745 tokens of rules about *how* the agent looks things up and waits. Recon in one pass instead of five pokes at the repo; 50-line keyhole reads when it only needs to see something, never truncating data it will actually transform; every dependency probed in one command instead of discovered one crash at a time; the task's own check as the definition of done; and polling a build every 30 seconds rather than every second — which on some platforms turned out to be nearly half of all steps. Measured rather than claimed: 80 paired SkillsBench tasks give −17.9% median cost and −22% tokens with quality unchanged (7 better / 5 worse / 68 ties, sign p = 0.77), and a second run on Java SWE-bench through Codex CLI holds at −4.4% [−7.5, −1.5]. The finding worth more than the saving is about delivery: the same text injected saves ~18%, while left as a discoverable skill folder it saves nothing at all (−0.5%, n.s.) because agents burn steps finding `SKILL.md`. So it installs by appending to `CLAUDE.md` / `AGENTS.md` or through a `SessionStart` hook — not as a skill. ⚠️ Below this list's star floor, and in anyway: the floor stands in for evidence someone other than the author ran the thing, and a published paired A/B from a vendor that is not one person is that evidence directly.
- **[agentmemory](https://github.com/rohitg00/agentmemory)** — persistent memory for coding agents, benchmarked rather than asserted. The first survivor of a memory wave that produced six tools in a single radar week; most of the rest that promised to fix "the agent forgets between sessions" are still below the star floor.
- **[ai-memory](https://github.com/akitaonrails/ai-memory)** — the second survivor, and it solves the half the others skip: not memory inside one agent, but the **handoff between them**. Lifecycle hooks capture prompts, tool calls and session boundaries; on session end they compile into a "where you left off" block that the *next* agent gets injected at start. Quit Claude Code mid-task, open Codex in the same directory, and the architecture, the failed approaches and the open questions are already there. A single Rust binary speaking MCP, with hook and config formats for Claude Code, Codex, Cursor, Gemini CLI, OpenCode, Devin CLI, Pi and Crush — the widest harness matrix in this section. Capturing the assistant's own replies is double opt-in and off by default, which is the right default for anything that records your sessions.
- 🧪 **[rtk](https://github.com/rtk-ai/rtk)** — proxies CLI commands and hands the agent a compressed digest instead of a wall of output. The one tool here I have actually audited, because it keeps a `history.db` of every call and what it saved — which is honest of them and inconvenient for me. Across **31,933 commands in three months** the real number is **22.1%** (11.3M tokens of 51.1M), against a live counter that was showing 92.9%; the **median call saves zero**, and 80% of all savings came from 3.6% of the calls. It also decayed as I used it more — 54.7% and 56.1% in the first two months, 15.1% and 21.5% in the next two — and that was me, not the tool: I moved from calling it by hand on genuinely fat commands to a global hook that wrapped `git status`, `curl` and `echo` alike. **The cost that matters is not tokens.** `prettier --check` exited 1 while the digest cheerfully said clean, so the agent reformatted a file on the strength of it; `git diff > patch` produced a summary instead of a patch, and `git apply` answered `No valid patches in input`. Those are decisions taken on a lie. On test runs, 13.2% ended in re-running the same tests because the tracebacks had been eaten. ⚠️ **Check your version before believing any of that** — I measured 0.42.0, and the release notes for 0.43.0 (June 28) read `fix(grep): correctly handle all flag shapes` and `fix(core): propagate exit code instead of reporting ok`, which is precisely the 7,423 grep parse failures and precisely the `prettier` lie. After updating, flags parse, exit codes propagate, and truncation announces itself with `+537 more` and a path to the full log. What survives on my machine: `cat` / `head` / `tail`, `ls` / `find` / `tree`, `git diff`, `ps aux` / `du`, `gh` — five groups worth **71% of the savings at zero restarts** — with tests, linters, typecheckers, `curl`, `git status` and `git log` in `exclude_commands`. The rule the audit left behind, and it outlives this particular tool: **compress what a human reads with their eyes; never compress what gets parsed or decided on** — patches, exit codes, CI gates.
...
- **[Multica](https://github.com/multica-ai/multica)** — the same fleet, modelled as an issue tracker instead of a cockpit: the agent is assigned a ticket like a teammate, picks it up, comments as it goes, raises blockers and hands the result back for review, in one workspace with the humans. Twenty agent CLIs, self-hostable. It treats a different problem than the parallelism above — not "the agent is slow" but the babysitting tax, when five agents live in five terminal tabs, forget everything at the end of a session and make you explain the context a fourth time.
...
- **[humanizer](https://github.com/blader/humanizer)** — strips the tells of AI writing: inflated symbolism, promotional vocabulary, "-ing" pseudo-analysis, vague attribution, the rule of three, em-dash overdose, filler — the rules come from the WikiProject AI Cleanup guide, not from one person's taste. The part that makes it more than a linter is voice calibration: hand it something you wrote earlier and it matches your habits instead of flattening the text into house style, your sample overriding its own rules. Plain Markdown, so it installs into any harness — and the underrated use is not cleaning output after the fact but leaving it in context, where the thing it improves is what the agent writes *to you* mid-session, and the PR descriptions a human has to read.
